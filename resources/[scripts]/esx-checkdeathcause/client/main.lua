...
local granted = false
function GrantCheckDeathResoan()
	Citizen.CreateThread(function()
		Citizen.Wait(1000)
		while granted do
			local sleep = 3000
...
function Start(ped)
	...
	while checking do
		...
		if distance < 2.0 then
			DrawText3D(x,y,z, 'Baraye Barresi Jasad [~g~E~s~] ra feshar dahid', 0.4)
...
function Notification(x,y,z)
	...
	while (timestamp + 4500) > GetGameTimer() do
		Citizen.Wait(0)
		DrawText3D(x, y, z, 'Be inja Zarbe Khorde', 0.4)
		checking = false
	end
end
...
function OpenDeathMenu(player)
	...
	table.insert(elements, {label = 'Barresi Dalil Marg', value = 'deathcause'})
	table.insert(elements, {label = 'Peyda kardan mahale zarbe khorde', value = 'damage'})
	...
	function(data, menu)
		...
		if ac == 'damage' then
			...
			if success then
				...
				Notify('Mahale Zarbe Khordegi moshakhas nist')
			end
		end
