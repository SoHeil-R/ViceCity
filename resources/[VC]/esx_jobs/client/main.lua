...
AddEventHandler('esx_jobs:action', function(job, zone)
	...
	elseif zone.Type == "work" then
		...
		print(ESX.dump(zone))
		if zone.Item[1].db_name == "alive_chicken" or zone.Item[1].db_name == "slaughtered_chicken" or zone.Item[1].db_name == "packaged_chicken" then


		if not IsPedInAnyVehicle(playerPed, false) then

			local inventory = ESX.GetPlayerData().inventory
			local alive_chicken = 0
			local slaughtered_chicken = 0
			local packaged_chicken = 0
			
			for i = 1, #inventory, 1 do 

				if inventory[i].name == "alive_chicken" then
					alive_chicken = inventory[i].count
				end

				if inventory[i].name == "slaughtered_chicken" then
					slaughtered_chicken = inventory[i].count
				end

				if inventory[i].name == "packaged_chicken" then
					packaged_chicken = inventory[i].count
				end

			end

			if zone.Item[1].db_name == "alive_chicken" then	

				if alive_chicken < 20 then

					TriggerEvent("mythic_progbar:client:progress", {
						name = "catch_aliveChicken",
						duration = 3000,
						label = "Dar hale gereften morgh zende",
						useWhileDead = false,
						canCancel = true,
						controlDisables = {
						...
						if not status then
							...
							hintIsShowed = false
							isInMarker = false
							TriggerServerEvent('esx_jobs:giveShit', "alive_chicken", 1)
					...
					ESX.ShowNotification("Shoma fazaye khali baraye ~y~Morgh zende ~s~nadarid~")

				end

			elseif zone.Item[1].db_name == "slaughtered_chicken" then

				if slaughtered_chicken < 20 then

					if alive_chicken > 0 then

						TriggerEvent("mythic_progbar:client:progress", {
							name = "kill_chicken",
							duration = 5000,
							label = "Dar hale koshtan morgh zende",
							useWhileDead = false,
							canCancel = true,
							controlDisables = {
								disableMovement = true,
							...
							if not status then
								...
								hintIsShowed = false
								isInMarker = false
								TriggerServerEvent('esx_jobs:giveShit', "slaughtered_chicken", 1)
								ClearPedTasksImmediately(GetPlayerPed(-1))
						...
						ESX.ShowNotification("Shoma ~y~Morgh zende ~s~nadarid~")
					end

				else

					ESX.ShowNotification("Shoma fazaye khali baraye ~y~Morgh ~s~nadarid~")

				end

			elseif zone.Item[1].db_name == "packaged_chicken" then

				if packaged_chicken < 100 then

					if slaughtered_chicken > 0 then

						TriggerEvent("mythic_progbar:client:progress", {
							name = "package_chicken",
							duration = 4000,
							label = "Dar hale baste bandi morgh",
							useWhileDead = false,
							canCancel = true,
							controlDisables = {
								disableMovement = true,
							...
							if not status then
								...
								hintIsShowed = false
								isInMarker = false
								TriggerServerEvent('esx_jobs:giveShit', "packaged_chicken", 5)
								ClearPedTasksImmediately(GetPlayerPed(-1))
						...
						ESX.ShowNotification("Shoma ~y~Morgh ~s~nadarid~")
					end

				else

					ESX.ShowNotification("Shoma fazaye khali baraye ~y~Morgh baste bandi shode~s~nadarid~")
							...
							if playerPed == driverPed then
								...
								for i=1, #myPlate, 1 do
									if myPlate[i] == plate then
										...
										TriggerServerEvent('esx_jobs:VCcaution', "give_back", giveBack, 0, 0)
...
function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	hintToDisplay = 'no hint to display'
	hintIsShowed = false
	TriggerServerEvent('esx_jobs:VCcaution', 'take', vehicleCaution, spawnPoint, vehicle)
end
