...
function startScenario(anim)
	if anim == "world_human_leaning" then
		ESX.SetPlayerData('banim', 1)
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'animations_sub',
	{
		...
		local anim = data.current.value.anim
				...
				if canRun then
					...
					if ESX.GetPlayerData()['banim'] ~= 1 then
						...
						startAnimPos = GetEntityCoords(GetPlayerPed(-1), true)
					
					else
						ESX.ShowNotification("~r~Shoma nemitavanid dar hale anjam in animation kare digaro anjam dahid!")
					end
					

				else
					ESX.ShowNotification("~r~Lotfan Animation Spam Nakonid!")
				end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['X']) and IsInputDisabled(0) and not isDead then
			...
			if GetDistanceBetweenCoords(currentPos, startAnimPos) < 2 then
			ESX.Game.Teleport(GetPlayerPed(-1), startAnimPos)
			startAnimPos = nil
			end
		end
