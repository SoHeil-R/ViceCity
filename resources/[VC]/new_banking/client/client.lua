...
local atms = {
	{name="ATM", id=277, x=147.73, y=-1035.74, z=29.34},
	{name="ATM", id=277, x=145.95, y=-1035.17, z=29.34},
...
if bankMenu then
	...
	if nearBank() or nearATM() then
			DisplayHelpText("Barye Vorod ~INPUT_PICKUP~ Ra feshar dahid!~b~")
...
RegisterNUICallback('deposit', function(data)
	TriggerServerEvent('bank:VCdeposit', tonumber(data.amount))
	TriggerServerEvent('bank:balance')
end)
...
RegisterNUICallback('transfer', function(data)
	TriggerServerEvent('bank:VCtransfer', data.to, data.amountt)
	TriggerServerEvent('bank:balance')
end)
