...
function SetVehicleMaxMods6(vehicle)
  ...
  function OpenCloakroomMenu()
	  ...
	  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	  {
		  ...
		  ESX.TriggerServerCallback('esx_core:checkswat', function(isSwat)
				if isSwat then
					setUniform(data.current.value, playerPed)
		  ...
		  if data.current.value == 'sbullet_wear' or data.current.value == 'sboss_wear' or data.current.value == 'scommander_wear' or data.current.value == 'sslo_wear' or data.current.value == 'ssergeant_wear' or data.current.value == 'spo3_wear' or data.current.value == 'spo2_wear' or data.current.value == 'spo1_wear' or data.current.value == 'scadet_wear' then
			ESX.TriggerServerCallback('esx_core:checksheriff', function(isSheriff)
				if isSheriff then
					setUniform(data.current.value, playerPed)
					ESX.SetPlayerData('IsSwat', 0)
				else
	...
	ESX.TriggerServerCallback('esx_policejob:getArmoryWeapons', function(weapons)
	  ...
	  for i=1, #weapons, 1 do
			  if weapons[i].count > 0 then
						...
						if ESX.GetPlayerData()['IsSwat'] == 1 then
							if weapons[i].name == "WEAPON_CARBINERIFLE" then
...
AddEventHandler('esx_policejob:getuncuffed', function(playerheading, playercoords, playerlocation)
  ...
  Citizen.CreateThread(function()
	  while true do
		  ...
		  if IsControlJustReleased(1, Keys["U"]) and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
					SendPoliceDistressSignal()
		  end

		  if IsControlJustReleased(1, Keys["PAGEUP"]) and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
			SendBackup()
		  end
...
function SendBackup()
	...
	if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
		...
		ESX.TriggerServerCallback('esx_policejob:getIcName', function(name)
			...
			TriggerServerEvent("esx_policejob:soundplay", "demo", 0.5, PedPosition.x, PedPosition.y, nil, name, false)
