	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				ESX.Game.SpawnVehicle(Config.Zones.VehicleSpawnPoint.Pos, Config.Zones.VehicleSpawnPoint.Heading, function(vehicle)
					local playerPed = PlayerPedId()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
		{
			...
			ESX.Game.SpawnVehicle(data.current.model, Config.Zones.VehicleSpawnPoint.Pos, Config.Zones.VehicleSpawnPoint.Heading, function(vehicle)
				if data.current.model == "superd" or data.current.model == "stretch" then

					local playerPed = PlayerPedId()
					TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
					SetVehicleMods(vehicle, true, 88, 88, 88)
...
function IsInAuthorizedVehicle()
	...
	local deleteAbleVehicles = Config.AuthorizedVehicles.delete
	for i=1, #deleteAbleVehicles, 1 do
		if vehModel == GetHashKey(deleteAbleVehicles[i].model) then
			return true
		end
	end
