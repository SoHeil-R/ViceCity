...
local timing, isPlayerWhitelisted, inCapture = math.ceil(Config.Timer * 60000), false, false
...
Citizen.CreateThread(function()
	while true do
		...
		elseif IsPedShooting(playerPed) and Config.GunshotAlert then
			...
			local playerPed, petrolHash = GetPlayerPed(-1), GetHashKey('WEAPON_PETROLCAN')
			local hasCan = HasPedGotWeapon(playerPed, petrolHash, false)
			if hasCan then
				return
			end
