...
function notifyAlertSMS (number, alert, listSrc)
  if PhoneNumbers[number] ~= nil then
	local mess = 'From #' .. alert.numero  .. ' : \n' .. alert.message
  if alert.coords then
    loc = 'GPS: ' .. alert.coords.x .. ', ' .. alert.coords.y 
	end
    for k, _ in pairs(listSrc) do
      getPhoneNumber(tonumber(k), function (n)
        if n ~= nil then
          ...
          if loc then
            TriggerEvent('gcPhone:_internalAddMessage', number, n, loc, 0, function (smsMess)
              TriggerClientEvent("gcPhone:receiveMessage", tonumber(k), smsMess)
            end)
          end
        end
...
AddEventHandler('esx_phone:registerNumber', function(number, type, sharePos, hasDispatch, hideNumber, hidePosIfAnon)
  print('= INFO = Phone Number Registered ' .. number .. ' => ' .. type)
